...
export default function CalculatorPage() {
  ...
  const [modPolyError, setModPolyError] = useState<string | null>(
    null,
  );
  ...
  const nVal =
    Math.max(0, parseInt(expN || "0", 10) || 0);

  const validModPoly = modPoly !== 0;

  // We will fill these safely; never call gf* when modPoly is invalid.
  let aField = rawA;
  let bField = rawB;
  let result = 0;
  const steps: Step[] = [];
  ...
  if (validModPoly) {
    // Only reduce and compute when modulus is valid
    const aMod = gfMod(rawA, cfg);
    const bMod = gfMod(rawB, cfg);
    aField = aMod.value;
    bField = bMod.value;
  ...
  function handleCalculate() {
    ...
    push({
      ...
      modPoly,
      op,
      a: ["add", "sub", "mul", "div", "pow", "mod"].includes(
        op,
      )
        ? aField
        : undefined,
      b: ["add", "sub", "mul", "div"].includes(op)
        ? bField
        : undefined,
  ...
  function rerun(it: HistoryItem) {
    ...
    setOp(it.op);
    if (it.a !== undefined)
      setAHex(it.a.toString(16).toUpperCase());
    if (it.b !== undefined)
      setBHex(it.b.toString(16).toUpperCase());
            ...
            <section className="rounded-2xl border border-slate-800 bg-slate-900/60 backdrop-blur p-6">
              ...
              <div className="mb-4">
                ...
                <select
                  value={m}
                  onChange={(e) =>
                    onChangeM(parseInt(e.target.value))
                  }
              ...
              <div className="mb-5">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    // If input is empty, fall back to default irreducible for this m
                    if (!clean) {
                      const fallback = IRRED_DEFAULTS[m] ?? modPoly;
                      setModPoly(fallback);
                      return;
                    }
                    ...
                    // Never allow zero polynomial – also fall back to default
                    if (parsed === 0) {
                      const fallback = IRRED_DEFAULTS[m] ?? modPoly;
                      setModPoly(fallback);
                    } else {
                ...
                <div className="mt-1 text-xs text-slate-400">
                  ...
                  <span className="font-mono">
                    {asPolyString(modPoly)}
                  </span>{" "}
                  • Default AES for m=8 is 0x11B
                </div>
                {modPolyError && (
              ...
              <Button
                ...
                disabled={!validModPoly}
                className="mt-6 h-11 gap-2 bg-cyan-500 text-slate-900 hover:bg-cyan-400 disabled:opacity-60"
              >
                  ...
                  <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4 space-y-2 text-sm">
                    {steps.length === 0 && (
                      <div className="text-slate-400">
                        No step trace for this operation.
                      </div>
                    )}
                    {steps.map((s, i) => (
                      <div
                ...
                >
                  <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4">
                    <div className="flex items-center gap-2 text-slate-200 mb-2">
                      <NotepadText className="h-5 w-5 text-cyan-400" />{" "}
                      Mathematical Explanation
                    </div>
                    <p className="text-slate-300 leading-7">
                      In GF(2^m), addition and subtraction
                      are bitwise XOR. Multiplication uses
                      shift-and-add with reduction by the
                      chosen irreducible polynomial (e.g.,
                      AES 0x11B for m=8). Inverse is computed
                      via the Extended Euclidean Algorithm
                      over GF(2)[x]; power uses binary
                      exponentiation.
                    </p>
                  </div>
                </TabsContent>
