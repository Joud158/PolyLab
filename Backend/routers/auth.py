...
from ..schemas import BasicOK, LoginIn, SignupIn, UserOut  # make sure UserOut exists
...
def get_csrf(response: Response) -> dict:
    ...
    Frontend calls this before unsafe requests.
    """
...
def _verify_email_token(token: str, db: Session) -> None:
    """
    Helper that marks the user corresponding to a verification token as verified.
    """
...
def verify_email_page(token: str, db: Session = Depends(get_db)):
    ...
    # Fallback minimal page if no FRONTEND_ORIGIN is configured
    return """
    <html>
...
def login(payload: LoginIn, response: Response, db: Session = Depends(get_db)):
    """
    Authenticate a user, create a session and set the session + CSRF cookies.
    """
...
def read_me(current_user: User = Depends(get_current_user)):
    ...
    Frontend uses this to restore auth state.
    """
    return current_user
...
def reset_start(email: str, db: Session = Depends(get_db)):
    """
    Start password reset flow: send a reset email if the user exists.
...
def reset_confirm_page(token: str):
    ...
    Redirects to frontend reset page if configured, otherwise shows a minimal form.
    """
    if settings.FRONTEND_ORIGIN:
...
def reset_confirm(
    ...
    """
    Finish password reset using a token, either from form fields or query params.
    """
